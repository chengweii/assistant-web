    ...
    private void bindScheduleModel(Map<String, Object> model) throws Exception {
        ...
        if (!CollectionUtil.isEmpty(taskList)) {
            for (Task task : taskList) {
                ...
                if (Strings.isNullOrEmpty(task.startDate)) {
                    map.put("schedule_title", task.title);
                } else {
                    Date date = DateUtil.getDateFormat(task.startDate, DateFormatType.YYYY_MM_DD_T_HH_MM_SS_SSSZ);
                    map.put("schedule_title", task.title + "<br>" + DateUtil.getDateFormat(date, DateFormatType.YYYY_MM_DD_HH_MM_SS));
                }
